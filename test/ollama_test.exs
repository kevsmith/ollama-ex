defmodule OllamaTest do
  ...
  describe "chat2" do
    ...
    test "generates a structured response for a given prompt", %{client: client} do
      assert {:ok, res} =
               Ollama.chat(client,
                 model: "llama2",
                 messages: [
      ...
      assert res["done"]
      assert res["model"] == "llama2"
      message = Jason.decode!(res["message"]["content"])
      assert "Canada" = message["name"]
      assert "Ottawa" = message["capital"]
      assert ["English", "French"] = message["languages"]
    end
